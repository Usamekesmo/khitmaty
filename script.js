        ...
        snap.forEach(d => {
            ...
            if (m.isActive !== false) {
                ...
                const row = {
                    ...
                    'الحلقة': m.circleId || '',
                    'الجزء المطلوب': m.currentJuz || m.selectedJuz || '',
...
function exportUsersFullPDF() {
    ...
    db.collection('circleMembers').get().then(snap => {
        ...
        const headers = [
            ['#', 'الاسم', 'البريد', 'تاريخ الانضمام', 'الجزء المطلوب', 
                    ...
                    <table style="width:100%; border-collapse:collapse; font-size:12px;">
                        <thead>
                            <tr style="background:#1a4739; color:white;">
                                ...
                                <th style="text-align:right; padding:8px;">الجزء المطلوب</th>
...
function openCircleManagement() {
    if (!currentCircleId) {
        showToast("⚠️ أنت لست في حلقة", true);
        return;
    }
    
    if (!isCircleAdmin) {
        showToast("⚠️ ليس لديك صلاحية لإدارة هذه الحلقة", true);
        return;
    }
...
async function loadCircleManagementData() {
    try {
        const circleDoc = await db.collection('circles').doc(currentCircleId).get();
        if (circleDoc.exists) {
        ...
        const list = document.getElementById('circleMembersManagementList');
        list.innerHTML = '';
        ...
        for (let d of members.docs) {
            ...
            div.style.cssText = `border-right:4px solid ${hasReadToday ? '#22c55e' : '#ef4444'}; margin-bottom:8px;`;
            div.innerHTML = `
                <div style="flex:1;">
                    <strong>${escapeHtml(m.userName)}</strong>
                    ${m.isCircleAdmin ? ' 👑' : ''}
                    <br>
                    <small>📧 ${m.userEmail || 'غير مسجل'}</small>
                    <br>
                    <small>📖 ${totalParts} جزء | ${hasReadToday ? '✅ قرأ اليوم' : '❌ لم يقرأ اليوم'}</small>
                    <br>
                    <small>⚠️ غياب: ${m.absenceCount || 0} يوم</small>
                </div>
                <div>
                    ${!m.isCircleAdmin ? `<button onclick="makeCircleAdmin('${d.id}')" class="edit-btn" style="background:#8b5cf6; margin-bottom:5px; display:block; width:100%;">👑 جعل مدير</button>` : ''}
                    <button onclick="removeFromCircle('${d.id}')" class="delete-btn" style="display:block; width:100%;">🗑️ حذف</button>
                </div>
            `;
        ...
        if (members.size === 0) {
            list.innerHTML = '<div style="text-align:center; padding:20px; color:#666;">لا يوجد أعضاء في الحلقة</div>';
        }
...
function updateSpecificMemberList(members) {
    const select = document.getElementById('specificMemberList');
    select.innerHTML = '';
    members.forEach(d => {
        const m = d.data();
        const option = document.createElement('option');
        option.value = d.id;
        option.textContent = m.userName + ' (' + m.userEmail + ')';
...
async function regenerateInviteCode() {
    ...
    try {
        ...
        await db.collection('circles').doc(currentCircleId).update({
            inviteCode: newCode,
            updatedAt: new Date()
        });
        
        document.getElementById('circleInviteCodeDisplay').value = newCode;
        showToast('✅ تم تغيير رمز الحلقة بنجاح', false);
        
        await sendNotificationToMembers('🔑 تم تغيير رمز الحلقة', 'تم تغيير رمز الحلقة الجديد: ' + newCode);
        
    } catch (error) {
        console.error(error);
        showToast('حدث خطأ أثناء تغيير الرمز', true);
    }
}

async function makeCircleAdmin(memberId) {
    ...
    try {
        ...
        showToast('✅ تم جعل العضو مديراً للحلقة', false);
...
async function removeFromCircle(memberId) {
    if (!confirm('⚠️ هل أنت متأكد من حذف هذا العضو من الحلقة؟')) return;
    
    try {
        const memberDoc = await db.collection('circleMembers').doc(memberId).get();
        const member = memberDoc.data();
        
        if (!member) {
            showToast('العضو غير موجود', true);
            return;
        }
        ...
        await db.runTransaction(async t => {
            ...
            if (data) {
                ...
                const newAvail = [...data.availableJuz, member.selectedJuz].sort((a, b) => a - b);
        ...
        const circleRef = db.collection('circles').doc(currentCircleId);
        const circleDoc = await circleRef.get();
        await circleRef.update({
            memberCount: (circleDoc.data().memberCount || 1) - 1
        });
        
        showToast('✅ تم حذف العضو من الحلقة', false);
        await loadCircleManagementData();
        
    } catch (error) {
        console.error(error);
        showToast('حدث خطأ أثناء حذف العضو', true);
    }
}

async function sendNotificationToMembers(title, message) {
    try {
        const members = await db.collection('circleMembers')
            .where('circleId', '==', currentCircleId)
            .where('isActive', '==', true)
        ...
        for (let d of members.docs) {
            ...
            if (m.userId && m.userId !== currentUser.uid) {
                await addNotification(m.userId, title, message, 'circle');
                count++;
            }
        }
        
        console.log(`✅ تم إرسال الإشعار إلى ${count} عضو`);
    } catch (error) {
        console.error('خطأ في إرسال الإشعارات:', error);
    }
}

async function sendMessageToMembers(type) {
    const subject = document.getElementById('messageSubject').value.trim();
    const content = document.getElementById('messageContent').value.trim();
    const recipientType = document.getElementById('messageRecipientType').value;
    const specificMember = document.getElementById('specificMemberList').value;
    ...
    try {
        let query = db.collection('circleMembers')
            .where('circleId', '==', currentCircleId)
            .where('isActive', '==', true);
        ...
        if (type === 'app') {
            for (let d of members.docs) {
                const m = d.data();
                if (m.userId && m.userId !== currentUser.uid) {
                    await addNotification(m.userId, subject || 'رسالة من مدير الحلقة', content, 'circle');
                    count++;
                }
            }
            resultDiv.textContent = `✅ تم إرسال الرسالة إلى ${count} عضو في التطبيق`;
            ...
            for (let d of members.docs) {
                const m = d.data();
                if (m.userEmail && m.userId !== currentUser.uid) {
                    emails.push(m.userEmail);
                }
            }
            
            if (emails.length === 0) {
                resultDiv.textContent = '⚠️ لا يوجد بريد إلكتروني للأعضاء';
                resultDiv.className = 'message error';
                return;
            }
            
            resultDiv.textContent = `📧 تم إرسال البريد إلى ${emails.length} عضو (محاكاة)`;
            resultDiv.className = 'message success';
            console.log('📧 عناوين البريد:', emails);
            console.log('📝 الموضوع:', subject || 'رسالة من مدير الحلقة');
...
async function getShareText(juzNumber, readJuz = null) {
    ...
    const circleName = await getCircleName();
...
async function getCircleName() {
    ...
    try {
        const circleDoc = await db.collection('circles').doc(currentCircleId).get();
        if (circleDoc.exists) {
            const circleName = circleDoc.data().circleName || 'حلقة بدون اسم';
            localStorage.setItem(`circle_${currentCircleId}`, circleName);
            return circleName;
        }
        return 'حلقة غير معروفة';
    } catch (error) {
        console.error('خطأ في جلب اسم الحلقة:', error);
...
function openShareModal() {
    ...
    getShareText(juzNum, readToday).then(shareText => {
...
function filterCirclesUI(circles) {
    ...
    for (const circle of uniqueCircles) {
        ...
        // ✅ إزالة عرض رمز الحلقة من البطاقة
        card.innerHTML = `
            <h4>🔄 ${escapeHtml(circle.name)}</h4>
            <p>👥 ${circle.memberCount}/${MAX_CIRCLE_MEMBERS}</p>
            ${genderBadge}
            <p style="font-size:12px; color:#666; margin-top:8px;">📌 للانضمام، استخدم رمز الحلقة في الأعلى</p>
        `;
...
async function selectJuz(circleId, juz) {
    ...
    try {
        ...
        const isFirstMember = membersCount.size === 0;
        ...
        showMessage('juzSelectionMessage', `✅ تم اختيار الجزء ${juz}${isFirstMember ? ' 👑 أنت الآن مدير الحلقة!' : ''}`, false);
        ...
        showToast(`🎉 مرحباً بك في الحلقة!${isFirstMember ? ' أنت مدير الحلقة!' : ''}`, false);
...
async function loadUserData() {
    ...
    isCircleAdmin = currentMemberData.isCircleAdmin === true;
...
async function updateUI() {
    ...
    const manageBtn = document.getElementById('manageCircleBtn');
    if (manageBtn) {
        manageBtn.style.display = isCircleAdmin || currentUser?.email === ADMIN_EMAIL ? 'inline-block' : 'none';
    }
...
async function loadCircleInfo() {
    ...
    document.getElementById('circleInfo').innerHTML = `<div class="stat-card"><h3>🔄 ${escapeHtml(c.circleName)} ${icon}</h3><p>🔑 ${c.inviteCode}</p><p>👥 ${c.memberCount || 0}/${MAX_CIRCLE_MEMBERS}</p><p>📖 جزئك: ${currentMemberData.selectedJuz}</p>${isCircleAdmin ? '<p>👑 أنت مدير الحلقة</p>' : ''}</div>`;
    document.getElementById('circleActions').innerHTML = `<button onclick="copyInviteCode()" style="background:#3b82f6; width:100%; padding:12px; border-radius:25px; color:white; margin-bottom:10px;">📋 نسخ الرمز</button>${isCircleAdmin ? `<button onclick="openCircleManagement()" style="background:#8b5cf6; width:100%; padding:12px; border-radius:25px; color:white; margin-bottom:10px;">⚙️ إدارة الحلقة</button>` : ''}<button onclick="leaveCircle()" style="background:#ef4444; width:100%; padding:12px; border-radius:25px; color:white;">🚪 مغادرة</button>`;
    ...
    for (let d of members.docs) {
        ...
        list.innerHTML += `<div class="member-item" style="${hasReadToday ? 'border-right:4px solid #22c55e;' : 'border-right:4px solid #ef4444;'}">
            <span>${escapeHtml(m.userName)} ${m.userId === currentUser.uid ? '(أنت)' : ''} ${m.isCircleAdmin ? '👑' : ''}</span>
    ...
    for (let m of filtered) {
        ...
        cont.innerHTML += `<div class="admin-list-item" style="${hasReadToday ? 'border-right:4px solid #22c55e;' : 'border-right:4px solid #ef4444;'}">
            <div>
                ...
                <small>${m.isCircleAdmin ? '👑 مدير حلقة' : ''}</small>
            </div>
            <div>
...
// أحداث إدارة الحلقة
document.getElementById('manageCircleBtn')?.addEventListener('click', openCircleManagement);

document.getElementById('messageRecipientType')?.addEventListener('change', function() {
    const specificDiv = document.getElementById('specificMemberSelect');
...
// إغلاق مودال إدارة الحلقة
document.querySelector('#circleManagementModal .close-modal')?.addEventListener('click', function() {
    document.getElementById('circleManagementModal').style.display = 'none';
});
...
window.sendMessageToMembers = sendMessageToMembers;
